...
SETUP   LDA   #$32   		;Reset Game parameters
        STA   RSPOS		;Right ship position	
        STA   LSPOS             ;Left ship position
        CLR   RDCNT             ;Number of Right Droids destroyed
        CLR   LDCNT             ;Number of left Droids destroyed
        LDX   #$0000
        STX   RBPOS             ;Right right bolt position
        STX   LBPOS             ;Left right bolt position
        CLR   RBDIR             ;Right right bolt direction
        CLR   LBDIR             ;Left right bolt direction
        CLR   SBCNT             ;Middle scroll bar count control
        LDX   #$0112            ;System timer 		
        STX   SBHGT             ;Middle scroll bar block height control
...
MAINLP	JSR   MSB	        ;Scroll middle bar	
	...
	LDX   SPEED		;Slow things up
DELAY   LEAX  -1,X
        BNE   DELAY

        LDA   RDCNT             ;Get right droid destroyed count			
        CMPA  #$08              ;All destroyed?
        BEQ   WINLP1             ;Yes 
 
        JSR   LSFI              ;Check left fire button and draw bolt
        
        LDA   LDCNT             ;Get left droid destroyed count
        CMPA  #$08              ;All destroyed?
        BEQ   WINLP3             ;Yes
        ...
        ;Some one has won
        ;****************
WINLP1  LDA   RSPOS             ;Get right ship position
        LDX   #$0616
        BRA   WINLP2

WINLP3  LDA   LSPOS             ;Get left ship position
        LDX   #$0608
...
AGTXT	FCB   /     ANOTHER GAME (Y OR N) ?    /
	...
	;Ask user for speed of game
        ;**************************
ASKSPD  JSR   $A928		;Clear screen and HOME cursor	
        LDX   #$0500		;position to write text
        STX   <$88
        LDX   #SPDTXT		;point to text to be displayed		
        JSR   $B99C		;Output text string
AS1     JSR   $A1C1		;Scan keyboard - result in A register
        CMPA  #$30   		;Zero
        ...
        LDA   #$FA          
        MUL                 
        STD   SPEED		;Store result in SPEED
        JMP   SETUP		;Return
...
NBLP1   ADDB  #$03			
	...
	;Right Ship control
        ;******************
RSCTRL  JSR   $A9DE             ;Read ALL Joysticks			
        ...
        TST   RSPOS             ;Have we moved upto top border
        BEQ   RSDRW             ;No - Redraw ship
        DEC   RSPOS             ;Decrease position of right ship
        BRA   RSDRW             ;Redraw ship     
RSDOWN  LDA   RSPOS             ;Get current ROW of right hand ship
        CMPA  #$B6              ;Bottom border
        BCC   RSDRW             ;Have we gone past bottom border?
        INC   RSPOS             ;No - Increase right ship position
...
RSDRW1  LDA   ,Y+
	...
	;Left Ship control
        ;*****************
LSCTRL  JSR   $A9DE             ;Read ALL Joysticks
        ...
        TST   LSPOS             ;Have we move up to top border
        BEQ   LSDRW             ;No - redraw ship
        DEC   LSPOS             ;Decrease position of left ship
        BRA   LSDRW             ;Redraw ship
LSDOWN  LDA   LSPOS             ;get current row of left ship
        CMPA  #$B6              ;Bottom border
        BCC   LSDRW             ;Have we gone past bottom border?
        INC   LSPOS             ;No - increase left ship position
...
LSDRW1  LDA   ,Y+
        ...
        ;*******************************
RSFI    TST   RBDIR		;Is RH laser already been fired?
        BNE   RSFI1             ;Yes
...
RSFI2   LDA   #$FF		;Set direction of new right bolt (right to left)	
        STA   RBDIR             ;And store
        LDA   RSPOS             ;Calculate start position of right bolt
        LDB   #$20
        MUL
        LDX   #$0696            ;Start position of right bolt
        LEAX  D,X               ;Offset number of rows
        STX   RBPOS             ;Store position of right bolt

RSFI1   LDX   RBPOS             ;Get right bolt position
        LDA   #$80               
        STA   ,X                ;Erase right bolt from screen

        LDA   RBDIR             ;Get direction of right bolt             
...
RBHB1   CMPA  #$9F              ;CHECK: right bolt hit defense barrier colour 2?              
        ...
        CMPA  #$E0              ;?????
        BCC   RBDRAW
        LDA   RBDIR             ;right bolt hit droid  ??
        INCA
        BEQ   RBHB3
        LEAX  1,X
...
RBHB6   DECA
        ...
        BEQ   RBHB7
        INC   LDCNT             ;Increase Left Droid destroyed count
        BRA   RBRSET

RBHB7   INC   RDCNT             ;Increase Right Droid destroyed count
        BRA   RBRSET

RBDRAW  LDA   #$9A              ;Bolt Graphic
        STA   ,X                ;Draw on screen
        STX   RBPOS             ;Update position
        RTS
...
RBCHK   LDA   ,X                ;Load graphic at laser position			 
        ...
        STA   RBDIR             ;Store bolt direction
...
RBBP2   DECA
        ...
        LDA   #$37   
        STA   $FF23
RBCHKE  STX   RBPOS             ;Store bolt position
        RTS                     ;Return

        ;*****************************
        ;Left hand fire button pressed
        ;*****************************
LSFI    TST   LBDIR             ;Has LH laser already been fired?			 
        BNE   LSFI1             ;Yes
...
LSFI2   LDA   #$01              ;Set direction of left bolt (left to right)			
        STA   LBDIR             ;and store
        LDA   LSPOS             ;Claculate start psotion of left bolt
        LDB   #$20
        MUL
        LDX   #$0689            ;Set top most row psotion
        LEAX  D,X               ;Offset number of rows down
        STX   LBPOS             ;store position of lleft bolt
        
LSFI1   LDX   LBPOS             ;Get left bolt position
        LDA   #$80
        STA   ,X                ;Erase left bolt from screen
        LDA   LBDIR             ;Get direction of left bolt
...
LBHB7   INC   RDCNT             ;Increase right droid destroyed count
        BRA   LBRSET

LBDRAW  LDA   #$9A              ;Bolt graphic
        STA   ,X                ;Draw on screen
        STX   LBPOS             ;Update position
        RTS

LBRSET  CLR   LBDIR             ;Reset Left bolt indicator			
        RTS
...
LBCHK   LDA   ,X	        ;Load graphic at left bolt position
        ...
        STA   LBDIR             ;Store left bolt direction
        LDA   #$3F              ;Make beep sound
        STA   $FF23
        LDB   #$0A
        CLR   $FF20
...
LBBP2   DECA
        ...
        LDA   #$37   
        STA   $FF23
LBCHKE  STX   LBPOS             ;Store bolt position
        RTS                     ;Return
...
SHIPL	FCB   $80,$80	
	...
	FCB   $80,$80
        ...
        ;Variable stotage
        ;****************
RSPOS   FCB 0                   ;Position of right ship
LSPOS   FCB 0                   ;Position of left ship
RDCNT   FCB 0                   ;Right droid count
LDCNT   FCB 0                   ;Left droid count
RBPOS   FDB 0                   ;Right bolt position
LBPOS   FDB 0                   ;Left bolt position
RBDIR   FCB 0                   ;Right bolt direction
LBDIR   FCB 0                   ;left bolt direction
SBCNT   FCB 0                   ;Middle scroll bar control
SBHGT   FDB 0                   ;Middle scroll bar height       
SPEED   FDB 0                   ;Game speed            